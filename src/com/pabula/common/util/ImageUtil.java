...
public class ImageUtil {
	 ...
	 * @author wangjie
     * 根据高度 原比例 放缩图片
     * @param inputFile  输入文件全名（路径和名称）
     * @param outputPicName 输出文件全名（路径和名称）
     * @param ratio 放缩比例
     */	
    ...
    private  String getSuffix(String inputFile,String outFile){
    	...
    	if(!in.isFile()){
    		log.error("图片放缩错误:输入文件不存在!!!");
    		return "";
    	}
    	if(!outParent.isDirectory()){
    		log.error("图片放缩错误:输出文件路径不存在!!!");
    		return "";
    	}
    	if(!inName.contains(".")){
    		log.error("图片放缩错误:输入文件不合法!!!");
    		return "";
    	}
    	if(!outName.contains(".")){ 
    		log.error("图片放缩错误:输出文件名不合法!!!");
    		return "";
    	}
    	...
    	//因为上传的时候已经对文件类型进行了检查 此处省略文件类型检查
        return inSuffix;
    }
